...
# reference : http://blog.csdn.net/u014595019/article/details/52759104
...
import tf_rnn.ptb.reader as reader
...
class PTBModel(object):
  """The PTB model. 多层LSTM结构的构建，输入预处理，LSTM的循环，损失函数计算，梯度计算和修剪"""
  ...
  def __init__(self, is_training, config):
    ...
    with tf.device("/cpu:0"):
        ...
        inputs = tf.nn.embedding_lookup(embedding, self._input_data)     # shape 20*20*200  middle dim represent timestep
    if is_training and config.keep_prob < 1:
    ...
    with tf.variable_scope("RNN"):
        for time_step in range(num_steps):
            ...
            (cell_output, state) = cell(inputs[:, time_step, :], state)   #cell_output shape 20*200    state is a tuple contains two lstm cell state state is consistence of c and h, cell_output meanings h
            outputs.append(cell_output)
    output_1 = tf.concat(1, outputs)
    output = tf.reshape(output_1, [-1, size])        # shape is 400*200
...
def run_epoch(session, m, data, eval_op, verbose=False):
  ...
  costs = 0.0
  iters = 0
  state = (m.initial_state[0].c.eval(), m.initial_state[0].h.eval(), m.initial_state[1].c.eval(), m.initial_state[1].h.eval())
